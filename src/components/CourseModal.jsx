...
const CourseModal = ({ program, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-brand-navy/70 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl relative overflow-hidden animate-in fade-in zoom-in duration-300 m-auto mt-10 mb-10">
                ...
                {/* Scrollable Content */}
                <div className="flex-1 overflow-y-auto p-6 sm:p-8 bg-brand-light/50">
                    <div className="flex flex-wrap gap-4 mb-8 text-sm font-semibold text-brand-navy/70 border-b border-brand-navy/20 pb-6">
                        <span className="flex items-center gap-1.5 bg-white px-4 py-2 rounded-xl shadow-sm border border-brand-navy/10">
                    ...
                    <h3 className="text-xl font-bold mb-6 text-brand-navy/90">Course Curriculum Breakdown</h3>

                    <div className="space-y-5">
                        {program.modules?.map((mod, idx) => (
                            <div key={idx} className="bg-white rounded-2xl p-6 border border-brand-navy/10 shadow-sm hover:shadow-md transition">
                                <div className="flex justify-between items-start gap-4 mb-5">
                                    <h4 className="font-bold text-lg text-brand-navy">{mod.title}</h4>
                                ...
                                <ul className="grid sm:grid-cols-2 gap-y-3 gap-x-6">
                                    {mod.topics?.map((topic, i) => (
                                        <li key={i} className="flex items-start gap-2.5 text-sm text-brand-navy/70 font-medium">
                ...
                {/* Footer actions */}
                <div className="p-5 sm:p-6 bg-white border-t border-brand-navy/10 shrink-0 flex justify-end gap-4 shadow-[0_-10px_40px_rgba(0,0,0,0.03)] relative z-20">
                    <button onClick={onClose} className="px-6 py-2.5 rounded-xl font-bold text-brand-navy/60 hover:bg-gray-100 transition">
                        Close
                    </button>
